...
* Single GMM equality test *
xi: gmm	(eq1: m_anemia - $vars_eq1) ///
    (eq2: m_anemia - $vars_eq2) [pw=factor_pond], ///
    instruments(eq1: $instrument_1 $controls $fe)  ///
    instruments(eq2: $instrument_5 $controls $fe)  ///
...
* Full specification *
cd "$root\3. Output\Tables\Appendix"

local j = 1
foreach y in $outcomes {
	
	if `j' == 1 {
	qui reg `y' w_bargain $controls $fe [pw=factor_pond], r
	outreg2 using myfile, tex replace label keep(w_bargain $controls) addtext(Year FE, Yes, Region FE, Yes, Wealth FE, Yes) bdec(3) sdec(3)
	qui ivregress 2sls `y' $controls $fe (w_bargain = $instrument) [pw=factor_pond],  r
	outreg2 using myfile, tex append label keep(w_bargain $controls) addtext(Year FE, Yes, Region FE, Yes, Wealth FE, Yes) bdec(3) sdec(3)
	}
